...
def convert_verse_to_dict(verse_row: pd.Series) -> Dict:
    """
    ...
    return {
        ...
        "themes": [verse_row['topic_name']],  # 주제는 리스트로
        "symbols": [],  # 나중에 매핑 필요
...
def load_all_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    Returns:
        (bible_objects_df, detection_labels_df, topic_weights_df, verse_bank_df)
    """
    print("=" * 60)
    print("데이터 로딩 시작...")
    print("=" * 60)
    ...
    return bible_objects_df, detection_labels_df, topic_weights_df, verse_bank_df
